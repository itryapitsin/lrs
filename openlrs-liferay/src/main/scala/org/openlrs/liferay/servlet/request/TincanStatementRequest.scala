...
import org.openlrs.serializer.{StatementSerializer, AgentSerializer}
import org.openlrs.xapi.{Statement, FormatType, Agent}
...
class TincanStatementRequest(r: HttpServletRequest)
  ...
  def agent = optional(Agent) match {
    case Some(value) => Some(JsonHelper.fromJson[Agent](value, new AgentSerializer))
    case None => None
  }
